...
class DDIMSampler(object):
    ...
    def decode(self, x_latent, cond, t_start, unconditional_guidance_scale=1.0, unconditional_conditioning=None,
               use_original_steps=False, mask=None, init_latents=None):
        ...
        for i, step in enumerate(iterator):
            ...
            x_dec, e_t = self.p_sample_ddim(x_dec, cond, ts, index=index, use_original_steps=use_original_steps,
                                          unconditional_guidance_scale=unconditional_guidance_scale,
                                          unconditional_conditioning=unconditional_conditioning)
            e_ts.append(e_t)
            
            if mask is not None: 
                x_dec = mask*x_dec+(1-mask)*init_latents
        e_ts = torch.stack(e_ts, dim=1)
        e_ts = e_ts.mean(dim=1)
        return x_dec, e_ts