...
# optimize the pipeline, use the first stage's z_ensembles
...
def make_convolutional_sample_with_eps_conditional(model, custom_steps, eta, x_T, skip_steps, eps_list,
    ...
    # print('intermediates', len(intermediates['x_inter']), intermediates['x_inter'][1].shape,eps_list.shape, skip_steps, custom_steps)
...
def tensor2pil(x):
    x = PIL.Image.fromarray((torch.clamp(x,min=0,max=1)*255).squeeze(0).permute(1,2,0).cpu().to(torch.uint8).detach().numpy())
    return x.convert("RGB")

def tensors2pil_list(x):
    x = (torch.clamp(x,min=0,max=1)*255).permute(0,2,3,1).cpu().to(torch.uint8).detach().numpy()
    x = [PIL.Image.fromarray(x[i]).convert("RGB") for i in range(x.shape[0])]
    return x
                ...
                for encoder_unconditional_guidance_scale in self.encoder_unconditional_guidance_scales:
                    for skip_steps in self.skip_steps:
                        with torch.no_grad():
                            ...
                            # print('zzz', z.shape, z_list[0].shape)
                            z_ensemble.append(z)
...
class DualCycleDiffusion(torch.nn.Module):
    def __init__(self, 
        ...
        self.sds = SDStochasticTextWrapper(source_model_type, custom_steps, eta, white_box_steps, skip_steps,
                ...
                for e_i in range(len(sds_encoder_scales_ori)):
                    ...
                    for s_i in range(len(sds_skip_steps_ori)):
                        ...
                        for d_i in range(len(sds_decoder_scales_ori)):
                            ...
                            mask = mask_generator.cal_mask_with_tensor(init_image, img).to(self.device)
    ...
    def get_img_with_mask(self, init_img, encode_text, decode_text, mask, codes, return_all=False):
        sds = self.sds
        sds_encoder_scales_ori = sds.encoder_unconditional_guidance_scales
        sds_decoder_scales_ori = sds.decoder_unconditional_guidance_scales
        sds_skip_steps_ori = sds.skip_steps
                ...
                for s_i in range(len(sds_skip_steps_ori)):
                    ...
                    for d_i in range(len(sds_decoder_scales_ori)):
                        ...
                        for s_i2 in range(s_i2_start, len(sds_skip_steps_ori)):
                            ...
                            sds.skip_steps = [skip_steps]
                            if return_all:
                                imgs_all.append(img.detach().cpu())
                            for j in range(bzs):
                                if score[j] > scores[j]:    
                                    scores[j] = score[j]
                                    imgs[j] = img[j]
        ...
        sds.skip_steps = sds_skip_steps_ori
        if return_all:
            return imgs, imgs_all
        return imgs    


    def forward(self, image, encode_text, decode_text, return_mask=False, return_img=False, return_all=False):
        precision_scope = autocast if self.precision == "autocast" else nullcontext
        with precision_scope("cuda"):
            with torch.no_grad():
                init_img = image
                ...
                if return_all:
                    img, imgs_all = self.get_img_with_mask(init_img, encode_text, decode_text, mask, codes, return_all=return_all)
                else:
                    img = self.get_img_with_mask(init_img, encode_text, decode_text, mask, codes, return_all=return_all)

                
        if return_mask:
            if return_img and return_all:
                return img, mask_list, mid_img_list, imgs_all
            if return_img:
